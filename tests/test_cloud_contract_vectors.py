...
from backend.cloud_session import (
    ...
    parse_public_job_cancel_response_v1,
    parse_public_job_detail_response_v1,
    parse_public_job_event_snapshot_response_v1,
    parse_public_job_list_response_v1,
...
JOB_PARSERS = {
    "job_list_response_v1": parse_public_job_list_response_v1,
    "job_detail_response_v1": parse_public_job_detail_response_v1,
    "job_event_snapshot_response_v1": parse_public_job_event_snapshot_response_v1,
    "job_cancel_response_v1": parse_public_job_cancel_response_v1,
}
...
def test_python_client_matches_all_public_desktop_job_vectors():
    fixture = json.loads(JOB_VECTOR_PATH.read_text(encoding="utf-8"))
    assert fixture["schemaVersion"] == 1
    assert fixture["contract"] == "lac.public.desktop-jobs"
    for vector in fixture["vectors"]:
        parser = JOB_PARSERS[vector["parser"]]
        if vector["valid"]:
            parser(vector["value"])
        else:
            with pytest.raises(
                (CloudSessionError, TypeError, ValueError),
                match="invalid_response",
            ):
                parser(vector["value"])


def test_desktop_job_vector_matches_the_canonical_semantic_digest():
    fixture = json.loads(JOB_VECTOR_PATH.read_text(encoding="utf-8"))
    canonical = json.dumps(
        fixture,
        sort_keys=True,
    ...
    assert hashlib.sha256(canonical).hexdigest() == CANONICAL_JOB_VECTOR_SHA256
...
def test_local_cloud_and_desktop_job_vectors_are_identical_when_co_located():
    cloud_vector = (
        ROOT.parent
        / "lac-cloud"
        / "packages"
        / "contracts"
        / "test-vectors"
        / JOB_VECTOR_PATH.name
    )
    if not cloud_vector.exists():
        pytest.skip("lac-cloud sibling checkout is not present")
    assert JOB_VECTOR_PATH.read_bytes() == cloud_vector.read_bytes()
