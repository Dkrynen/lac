...
def test_agent_unsupported_opencode_exits_with_clean_remediation(
    ...
    monkeypatch.setattr(
        ...
        lambda project, **kwargs: (_ for _ in ()).throw(
            OpenCodeUnsupportedVersion(
                "OpenCode 1.19.0 is installed; use 1.18.9."
            )
        ),
    )

    with pytest.raises(SystemExit) as exc:
        cli.cmd_agent(SimpleNamespace(dir=str(tmp_path), context=None, model=None, reselect=False))
