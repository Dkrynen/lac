...
- **Task 3 is a new non-Python surface** (Cloudflare Worker in JS/TS + `wrangler` + `vitest`/`miniflare`), built + unit-tested locally with a mocked Polar `fetch` and emulated R2. It replicates lac-pro's Polar validate call — `POST https://api.polar.sh/v1/customer-portal/license-keys/validate`, body `{key, organization_id: "c3771fa4-19b0-4f29-a444-0aa52b0daf36"}`, **a real `User-Agent` header is REQUIRED** (Polar's WAF 403s the default), `status === "granted"` = valid.
